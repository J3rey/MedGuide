  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (errorMessage.includes('quota') || errorMessage.includes('429')) {
        res.status(429).json({
          response:
            "I'm currently experiencing high demand. The AI service has reached its daily limit. Please try again in a few hours, or consult your healthcare provider for immediate medication information.",
